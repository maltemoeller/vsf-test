...
const compositionApiModule = function() {
  ...
  const {dst: pluginDst} = this.addTemplate({
    src: path.resolve(libRoot, "templates", "plugin.js"),
  ...
  const {dst: metaPluginDst} = this.addTemplate({
    src: path.resolve(libRoot, "templates", "meta.js"),
    fileName: path.join("composition-api", "meta.js")
  });
  ...
  const {dst: entryDst} = this.addTemplate({
    ...
    options: {
      ...
      staticPath: normalize__default['default'](staticPath),
      publicPath: normalize__default['default'](path.join(((_c = this.options.router) == null ? void 0 : _c.base) || "", "/")),
      globalContext,
      globalNuxt
    }
  });
  ...
  this.extendBuild((config) => {
    ...
    config.resolve.alias["@nuxtjs/composition-api"] = path.resolve(this.options.buildDir || "", entryDst);
  });
  ...
  if (!(this.nuxt.options.buildModules || []).includes("@nuxtjs/pwa") && !this.nuxt.options.modules.includes("@nuxtjs/pwa")) {
    this.options.plugins.push(path.resolve(this.options.buildDir || "", metaPluginDst));
